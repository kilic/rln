  ...
  E: Engine,
{
  pub fn new_from_alloc(an: num::AllocatedNum<E>) -> Self {
    return Element { an: Some(an), nu: None };
  }

  pub fn new_from_num(nu: num::Num<E>) -> Self {
    return Element { an: None, nu: Some(nu) };
  }
  ...
  pub fn allocate<CS: ConstraintSystem<E>>(&self, mut cs: CS) -> Result<num::AllocatedNum<E>, SynthesisError> {
    match self.nu.clone() {
      Some(nu) => {
        let v = num::AllocatedNum::alloc(cs.namespace(|| "allocate num"), || {
          nu.get_value().ok_or_else(|| SynthesisError::AssignmentMissing)
        })?;
        cs.enforce(
  ...
  E: Engine,
{
  ...
  pub fn round_type(&self) -> RoundType {
    let a1 = self.params.full_round_half_len();
    let (a2, a3) = (a1 + self.params.partial_round_len(), self.params.total_rounds());
    if self.number < a1 {
      RoundType::Full
    ...
    for i in 0..if ctx.is_full_round() { ctx.width() } else { 1 } {
      ...
      let si = {
        match self.elements[i].allocated() {
          Some(an) => an,
          None => self.elements[i].allocate(cs.namespace(|| format!("alloc sbox input {}", i)))?,
        }
      };
      let si2 = num::AllocatedNum::alloc(cs.namespace(|| format!("square with round constant {}", i)), || {
